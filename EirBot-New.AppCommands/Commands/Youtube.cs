...
namespace EirBot_New.Events {
	...
	public static class YoutubeEvents {
		...
		private static readonly Dictionary<ulong, YTSearchData> storedSearchResults = new();

		private static readonly Dictionary<int, string> buttonEmojis = new() {
			[0] = ":one:",
			[1] = ":two:",
			[2] = ":three:",
			[3] = ":four:",
		...
		public async static Task ButtonClicked(DiscordClient client, ComponentInteractionCreateEventArgs args) {
			...
			var found = storedSearchResults.TryGetValue(args.Message.Id, out activePicker);
			if (found && activePicker.context.User == args.User)
				await ChangeVideo(activePicker, newSelection);
		}

		public async static Task<YTSearchData?> NewSearchData(InteractionContext context, string query) {
		...
		public async static Task ChangeVideo(YTSearchData data, int newSelection) {
			// Set up buttons
			DiscordButtonComponent[] buttons = new DiscordButtonComponent[5];

			var id = (await data.context.GetOriginalResponseAsync()).Id;
			for (var i = 0; i < data.results.Length; i++) {
				var butt = new DiscordButtonComponent(
					ButtonStyle.Secondary,
					"youtube_queryLink_" + id + "_" + i,
					data.results[i].title,
					newSelection == i,
					new(DiscordEmoji.FromName(data.context.Client, buttonEmojis[i]))
				);
				buttons[i] = butt;
			}

			var wb = new DiscordWebhookBuilder().WithContent(data.results[newSelection].url);
		...
		private static videoData ParseResult(SearchResult result) {
			...
			var url = YOUTUBE_VIDEO_PREFIX + json["id"]?["videoId"]?.ToString();
...
namespace EirBot_New.AppCommands {
	...
	public partial class GenericCommands : AppCommandGroupBase {
		...
		public async static Task ytSearch(InteractionContext context, [Option("Query", "Search text.")] string query) {
			...
			var data = await YoutubeEvents.NewSearchData(context, query);
			if (data != null)
