...
public partial class StarboardCommands : AppCommandGroupBase {
	...
	public async static Task List(InteractionContext context) {
		...
		var settings = StarboardEvents.GetSettings(context.Client, context.Guild);
		if (settings == null) {
			await context.DeleteResponseAsync();
			return;
		}

		var starboardChannel = context.Guild.GetChannel(settings.Value.channelID);
		List<string> ignoredChannelNames = new();
		foreach (var id in settings.Value.ignoredChannels)
		...
		await context.EditResponseAsync(new DiscordWebhookBuilder()
			.AddEmbed(new DiscordEmbedBuilder()
				...
				.WithDescription(
					":gear: Enabled: "
					+ (starboardChannel != null && !string.IsNullOrEmpty(starboardChannel.Name) ? ":white_check_mark:" : ":x:")
					+ "\n"
					+ ":hash: Starboard channel: "
					+ (starboardChannel != null ? starboardChannel.Mention : ":x:")
					+ "\n"
					+ ":no_bell: Ignored channels: "
					+ (settings.Value.ignoredChannels.Count > 0 ? string.Join(", ", ignoredChannelNames) : ":x:")
					+ "\n"
					+ ":star: Minimum stars: "
					+ settings.Value.minStars
					+ "\n"
					+ ":underage: Allow posts from NSFW channels: "
					+ (settings.Value.allowNSFW ? ":white_check_mark:" : ":x:")
					+ "\n"
					+ ":index_pointing_at_the_viewer: Count stars from message author: "
					+ (settings.Value.allowSelfStar ? ":white_check_mark:" : ":x:")
					+ "\n"
					+ ":hammer: Remove on message delete: "
					+ (settings.Value.removeWhenDeleted ? ":white_check_mark:" : ":x:")
					+ "\n"
					+ ":dizzy: Remove when star count falls below minimum stars: "
					+ (settings.Value.removeWhenUnstarred ? ":white_check_mark:" : ":x:")
					+ "\n"
					+ ":robot: Use webhook for starboard messages: "
					+ (settings.Value.useWebhook ? ":white_check_mark:" : ":x:")
				)
			)
		);
	}
	...
	public async static Task MinStars(InteractionContext context, [Option("minimum-stars", "Minimum star amount to add a message to starboard.", false), MinimumValue(1), MaximumValue(25)] int minStars) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
			return;
		}
	...
	public async static Task AllowNSFW(InteractionContext context, [Option("allow-NSFW", "Whether messages from NSFW channels should be posted in the starboard.", false)] bool allowNSFW) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
	...
	public async static Task AllowSelfStar(InteractionContext context, [Option("allow-self-star", "Whether the message sender's own star reaction counts towards the starboard.", false)] bool allowSelfStar) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
	...
	public async static Task RemoveWhenUnstarred(InteractionContext context, [Option("remove-when-unstarred", "Whether starboard messages should be removed when star count falls below the minimum star count.", false)] bool removeWhenUnstarred) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
	...
	public async static Task RemoveWhenDeleted(InteractionContext context, [Option("remove-when-deleted", "Whether starboard messages should be removed when original message is deleted.", false)] bool removeWhenDeleted) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
	...
	public async static Task UseWebhook(InteractionContext context, [Option("use-webhook", "Whether a webhook should be used when posting to the starboard", false)] bool useWebhook) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
	...
	public async static Task Channel(InteractionContext context, [Option("channel", "Channel to post starboard messages in.", false), ChannelTypes(ChannelType.Text)] DiscordChannel channel) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
		...
		var botPermsInChannel = channel.PermissionsFor(await context.Guild.GetMemberAsync(context.Client.CurrentUser.Id));
		...
		var hook = await Util.GetWebhook(context.Client, channel);
		if (hook != null)
	...
	public async static Task IgnoreChannel(InteractionContext context, [Option("channel", "Channel to change ignore status of.", false), ChannelTypes(ChannelType.Text)] DiscordChannel channel, [Option("ignore", "Whether to ignore this channel in starboard operations.", false)] bool ignore = true) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
		...
		} else if (serverData.starboardSettings.ignoredChannels.Contains(channel.Id))
	...
	public async static Task Disable(InteractionContext context) {
		await context.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral());
		var serverData = ServerData.GetServerData(context.Client, context.Guild);
		if (serverData == null) {
			await context.DeleteResponseAsync();
