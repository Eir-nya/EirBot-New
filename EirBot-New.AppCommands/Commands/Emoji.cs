...
public partial class EmojiCommands : AppCommandGroupBase {
	...
	public async static Task Send(InteractionContext context, [Option("Search", "Text to search for emojis with.")] string search = "") {
		...
		// Get all emojis
		var id = (await context.GetOriginalResponseAsync()).Id;
		var picker = new EmojiEvents.EmojiPickerData(context, id, await EmojiEvents.GetAllEmoji(context.Client, context.Guild, search));
